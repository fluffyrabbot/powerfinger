...
typedef struct {
    uint8_t buttons;    // Up to 5 mouse buttons after role + gesture mapping
...
// Update one cached gesture action. Unsupported or unknown trigger/action
// pairs are ignored so callers can validate separately and keep this path
// branch-light inside the hot USB composition loop.
