...
hal_status_t calibration_run(void)
{
    for (int attempt = 0; attempt <= CALIBRATION_MAX_RETRIES; attempt++) {
        ...
        if (total_var > threshold_sq) {
            LOG_W("motion detected during calibration (var=%ld), retry %d/%d",
                  (long)total_var, attempt + 1, CALIBRATION_MAX_RETRIES);
            if (attempt < CALIBRATION_MAX_RETRIES) {
                hal_timer_delay_ms(CALIBRATION_RETRY_DELAY_MS);
                continue;
            }
            // Max retries exhausted — use zero offset
            LOG_W("calibration failed after %d retries, using zero offset",
