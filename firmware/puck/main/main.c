...
//
// This is structurally identical to the ring entry point. The puck shares
// all components via EXTRA_COMPONENT_DIRS; only log strings and Kconfig
// defaults (device name, sleep timeout) differ.
...
static const char *TAG = "powerpuck";
...
void app_main(void)
{
    ESP_LOGI(TAG, "PowerPuck firmware starting");
    ...
    if (settings_rc != HAL_OK && settings_rc != HAL_ERR_INVALID_ARG) {
        ESP_LOGW(TAG, "settings init failed (%d) — using runtime defaults only",
                 settings_rc);
    ...
    ESP_LOGI(TAG, "PowerPuck firmware ready");
    ...
    while (1) {
        ...
        if (ring_settings_needs_flush() &&
            ...
            if (flush_rc == HAL_OK) {
                ...
                ESP_LOGW(TAG, "settings flush failed (%d) — retrying in %d ms",
