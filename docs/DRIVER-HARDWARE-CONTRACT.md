...
| Charge-enable MOSFET path | `CONFIG_POWERFINGER_CHARGE_ENABLE_PIN`, `power_manager.c` | Boards that claim firmware charge cutoff need a GPIO-controlled logic-safe gate path. The R30 P0 board pass sets this Kconfig value to `-1` because fixture VBUS feeds TP4054 `VCC` through the non-BOM service jumper. | If unpopulated or Kconfig-disabled, charging may still occur electrically, but firmware charge cutoff is not present. | Blocks Gate 3 for enclosure charging. |
