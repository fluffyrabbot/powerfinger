...
physical-check coupons and matching previews:
  ...
  R30 service-access coupon for the off-board service-pad fixture path
...
After real first-sweep checks are recorded, ingest the USB-HUB rows back into
the checked-in result ledger with:
  scripts/ingest-usb-hub-coupon-results.py <FIRST-SWEEP/PHYSICAL-CHECK-WORKSHEET.md>

Ingest the R30 rows back into the checked-in ring result ledger with:
  scripts/ingest-r30-ring-coupon-results.py <FIRST-SWEEP/PHYSICAL-CHECK-WORKSHEET.md>
EOF
}
...
assert_file_not_contains() {
    ...
    if grep -Fq "$pattern" "$file"; then
        die "unexpected stale text in $file: $pattern"
    fi
}
...
assemble_first_sweep() {
    ...
    for rel_path in "${first_sweep_assets[@]}"; do
        copy_first_sweep_file "$rel_path"
    done

    {
        printf '<!-- SPDX-License-Identifier: CERN-OHL-S-2.0 -->\n'
        printf '# First-Sweep Physical Coupon Worksheet (Blank)\n\n'
        printf 'Generated by `scripts/generate-first-board-mechanical-packet.sh --first-sweep` at `%s`.\n\n' "$generated_at"
        printf 'Use this worksheet only after printing the four first-sweep STLs from `FIRST-SWEEP/` and checking them against real hosts, plugs, boards, board blanks, tools, or fixtures. Leave result fields blank until physical checks are complete.\n\n'
        printf 'For USB-HUB and R30 rows, accepted `Result` values are `pass`, `fail`, `partial`, and `blocked`. Any non-blank result must include the fixture, host, board, tool, or coupon print used plus an observation or photo reference. Then run the matching ingest helper from the repository root: `scripts/ingest-usb-hub-coupon-results.py %s` for USB-HUB rows and `scripts/ingest-r30-ring-coupon-results.py %s` for R30 rows.\n\n' "${first_sweep_output#$repo_root/}/PHYSICAL-CHECK-WORKSHEET.md" "${first_sweep_output#$repo_root/}/PHYSICAL-CHECK-WORKSHEET.md"
        ...
        printf '\nIngest real USB-HUB observations into `hardware/shared/USB-HUB/COUPON-RESULTS.md` with `scripts/ingest-usb-hub-coupon-results.py %s` and real R30 observations into `hardware/ring/R30-OLED-NONE-NONE/COUPON-RESULTS.md` with `scripts/ingest-r30-ring-coupon-results.py %s`; this generated first-sweep folder is a print-queue-ready subset, not measured evidence.\n' "${first_sweep_output#$repo_root/}/PHYSICAL-CHECK-WORKSHEET.md" "${first_sweep_output#$repo_root/}/PHYSICAL-CHECK-WORKSHEET.md"
...

    {
        ...
        printf 'This is the print-queue-ready starting bundle for the first physical sweep. It isolates the cheap, high-signal checks before printing larger or less-informative coupon sets, and includes four STLs plus matching preview PNGs and OpenSCAD logs.\n\n'
        printf '%s\n' '- Start with USB-HUB host-fit and clamp-alignment checks.'
        printf '%s\n' '- Start with R30 off-board service-pad access and board-retention checks.'
        printf '\nDo not fill result fields from these files alone. Print, check, then run `scripts/ingest-usb-hub-coupon-results.py %s` for the USB-HUB rows and `scripts/ingest-r30-ring-coupon-results.py %s` for the R30 rows before changing evidence docs.\n' "${first_sweep_output#$repo_root/}/PHYSICAL-CHECK-WORKSHEET.md" "${first_sweep_output#$repo_root/}/PHYSICAL-CHECK-WORKSHEET.md"
    ...
    first_sweep_manifest="$first_sweep_output/PACKET-MANIFEST.md"
    {
        ...
        printf 'Generated by `scripts/generate-first-board-mechanical-packet.sh --first-sweep` at `%s`.\n\n' "$generated_at"
        printf 'Scope: exactly four selected first-sweep STL coupons, matching previews, OpenSCAD logs, README, and a blank worksheet. This is the print-queue-ready starting bundle, not physical evidence.\n\n'
    ...
    validate_first_sweep
}

validate_first_sweep() {
    local expected_file_count=$(( ${#first_sweep_assets[@]} + 3 ))
    local actual_file_count

    for rel_path in "${first_sweep_assets[@]}"; do
        [[ -s "$first_sweep_output/$rel_path" ]] || die "missing or empty first-sweep artifact: $rel_path"
    done

    actual_file_count="$(find "$first_sweep_output" -type f | wc -l | tr -d '[:space:]')"
    [[ "$actual_file_count" == "$expected_file_count" ]] || \
        die "first-sweep file count drift: expected $expected_file_count, found $actual_file_count"

    assert_file_contains "$first_sweep_output/README.md" "print-queue-ready starting bundle"
    assert_file_contains "$first_sweep_output/PHYSICAL-CHECK-WORKSHEET.md" "not measured evidence"
    assert_file_contains "$first_sweep_output/PACKET-MANIFEST.md" "exactly four selected first-sweep STL coupons"
    assert_file_not_contains "$first_sweep_output/README.md" "USB-C"
    assert_file_not_contains "$first_sweep_output/PHYSICAL-CHECK-WORKSHEET.md" "USB-C"
    assert_file_not_contains "$first_sweep_output/PACKET-MANIFEST.md" "USB-C"
}
...

{
    ...
    write_bundle_summary_row \
        ...
        "hardware/shared/USB-HUB/COUPON-RESULTS.md, CONNECTOR-RETENTION-VERIFY.md, and MANIFEST.md"
    write_bundle_summary_row \
        "R30-OLED-NONE-NONE" \
        "$r30_output" \
        "scripts/generate-r30-ring-fit-coupons.sh" \
        "hardware/ring/R30-OLED-NONE-NONE/COUPON-RESULTS.md, STACKUP-VERIFY.md, and MANIFEST.md"
    if [[ "$first_sweep" -eq 1 ]]; then
        printf '\nFIRST-SWEEP/ is included as the print-queue-ready starting bundle and blank worksheet for the first physical coupon checks. It is not evidence until physical observations are recorded.\n'
    fi
...

{
    ...
    printf 'For USB-HUB rows, ingest real observations into `hardware/shared/USB-HUB/COUPON-RESULTS.md` with `scripts/ingest-usb-hub-coupon-results.py`; for R30 rows, ingest into `hardware/ring/R30-OLED-NONE-NONE/COUPON-RESULTS.md` with `scripts/ingest-r30-ring-coupon-results.py`. Use those source-controlled ledgers before changing evidence docs named in `PACKET-MANIFEST.md`.\n'
} > "$readme"
