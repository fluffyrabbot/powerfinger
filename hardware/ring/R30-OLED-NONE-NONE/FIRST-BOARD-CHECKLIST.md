...
- [x] Validate the charge path around `RT9080-33GJ5`, `20 kohm` charge resistor,
  NTC divider, and fixture-fed VBUS service jumper against
...
- [ ] Backfill schematic symbols to match the PCB pass and clear KiCad DRC
  (current snapshot in `kicad/CURRENT-VIOLATIONS.md`: ERC=0, DRC=23,
...
- [x] Cut the active `Q1`/`R4`/`Q2`/`R6` charge-gate from the first P0 and keep
  `Q1` as a non-BOM fixture-fed VBUS service jumper
- [x] Add `VBAT_SENSE` (`R7`/`R8`) and `VBUS_DETECT` (`R9`/`R10`) divider
  lines to the active BOM CSV
- [x] Add the `R7`-`R10` support parts to the local KiCad schematic and
  PCB so `VBAT_SENSE`/`VBUS_DETECT` are no longer pad-only claims
- [x] Record the packet recommendation for the first rigid P0:
  fixture-controlled charge VBUS, `R7`/`R8` = `100k`/`100k`, and
  `R9`/`R10` = `220k`/`100k`
- [x] BDFL accepted that packet recommendation for the first rigid P0:
  reintroducing an onboard charge-enable switch remains an explicit
...
- [x] Replace the onboard USB-C opening claim with first-pass left-edge
  off-board service-pad access; do not use the service interface as the
...
- [x] Add battery harness clearance from the actual `J_BAT` service-pad area plus a
...
- [x] Add quick-print OpenSCAD coupon/export modes for service-pad access,
  board rails/stops, lid compression pads, battery harness/service loop, and
  service-lid removal
- [x] Preserve glide-pad pockets and structural rim geometry on the lower shell
- [x] Replace the onboard JST-SH `J_BAT` body with same-net off-board battery
  service pads; the battery must still be replaceable through a harness or
  fixture, not direct cell solder
- [ ] Print/measure the off-board service-pad access, board rails/stops/lid
...
- [ ] Confirm the shell still supports battery replacement without destructive
  rework, service-fixture removal, antenna disturbance, or glide-rim
  disturbance
