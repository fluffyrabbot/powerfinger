...
- BDFL value lock for `R7`/`R8` (VBAT_SENSE divider), `R9`/`R10` (VBUS_DETECT
  divider), and `R11` (CHRG_STAT pull-up); the active BOM CSV carries starter
  recommendations only
- BDFL part lock for `Q2` (active BOM CSV currently lists 2N7002 with BSS138
  alt; logic-level load-switch substitution would drop both `Q2` and `R6`)
- Production routing for `VBAT_SENSE`, `VBUS_DETECT`, and `CHRG_STAT`; these
  are bring-up pads only until the new BOM lines are wired into the schematic
  and routed in the PCB
