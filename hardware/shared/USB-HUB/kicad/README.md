...
- `FABRICATION-OUTPUTS.md` — checked-in note for regenerating and reviewing the
  local Gerber, drill, POS, active-BOM/POS, and archive outputs
- `BOARD-HOUSE-OUTPUT-CONSTRAINTS.md` — board-house intake questions tied to
  the current generated-output state
...
- Local fabrication-review outputs are generated by
  `scripts/generate-usb-hub-fabrication-review.py` under
  `build-kicad/USB-HUB/fabrication-review/`. `FABRICATION-OUTPUTS.md` records
  the current output hashes and the active-BOM/POS review status; those outputs
  are reproducible review artifacts, not source-controlled fabrication release
  proof.
